...
def create_processor(config) -> DocumentProcessor:
    ...
    if hasattr(config, 'extractor') and getattr(config.extractor, 'use_local_llm', False):
        # Use local LLM extractor only if use_local_llm is True
        ...
        # Use OpenAI extractor if use_local_llm is False AND api_key is available
        ...
        # Handle case where neither local LLM nor OpenAI API key is configured
        logger.error("Entity extractor cannot be initialized: Configure either local LLM (USE_LOCAL_LLM=True) or provide OPENAI_API_KEY.")
        raise ValueError("Entity extractor configuration missing or invalid.")
