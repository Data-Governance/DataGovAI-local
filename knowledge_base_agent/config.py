...
class ExtractorConfig(BaseModel):
    ...
    @validator("use_local_llm", pre=True)
    def validate_use_local_llm(cls, v):
...
class Config(BaseModel):
    ...
    def from_env(cls, env_file: Optional[str] = None) -> 'Config':
        ...
        # Load environment variables, overriding existing ones from the file
        load_dotenv(env_file, override=True)
        # --- DEBUG PRINT REMOVED ---
        # print(f"DEBUG config.py: After load_dotenv(override=True), os.getenv('POSTGRES_CONNECTION') = [{os.getenv('POSTGRES_CONNECTION')}]")
        # --- END DEBUG PRINT ---
        ...
        for env_var, config_path in env_mapping.items():
            ...
            if value is not None:
                # --- DEBUG PRINT REMOVED ---
                # if env_var == 'POSTGRES_CONNECTION':
                #     print(f"DEBUG config.py: Setting config.storage.postgres_connection = [{value}]")
                # --- END DEBUG PRINT ---
                current = config
